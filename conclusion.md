...
* **Robust Backend Extraction (`web/routes/demo.py`)**: Added a blueprint endpoint `/api/demo/upload_video` which parses uploaded video files frame-by-frame using OpenCV. It implements a temporal sliding-window search (`+/- 400ms` window) to find faces when the exact target timestamp suffers from blinks or blur. It automatically registers the dataset session and triggers polynomial personalization training.
* **Simultaneous Recording (`web/templates/gaze_page.html` / `web/static/gaze_page.js`)**: Implemented browser-side `MediaRecorder` stream capturing, target timestamp logging, and auto-upload. Added a backup download feature that saves the recorded webm video and timeline JSON locally to the user's laptop to prevent data collection loss.
