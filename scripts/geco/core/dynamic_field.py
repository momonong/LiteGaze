...
class DynamicCognitiveField:
    ...
    """
    def __init__(self, word_boxes, base_cm, lambda_decay=0.5, sigma=30.0, use_ovp=True, is_L2=False, alpha_cm=None):
        """
        ...
        alpha_cm: If provided, dynamically interpolates between flat prior (alpha_cm=0) and full base_cm (alpha_cm=1).
        """
        ...
        if alpha_cm is not None:
            # Dynamically weight the cognitive mass priority based on reading proficiency
            self.base_cm = (1.0 - alpha_cm) * mean_cm + alpha_cm * base_cm_arr
        elif not is_L2:
